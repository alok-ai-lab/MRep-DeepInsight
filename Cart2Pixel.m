...
elseif strcmp(lower(Q.Method),'direct')==1
    Y=Q.data;
else
...
if strcmp(lower(Q.Method),'direct')==1
    [z,Cost,Time] = deepinsightTRep(Y,'Reference',Q.TightRep.Reference,...
        'EarlyStop',Q.TightRep.EarlyStop,'MaxIter',Q.TightRep.MaxIter,...
        'ShowRealTimeCostFunction',Q.TightRep.ShowRealTimeCostFunction,...
        'Dist',Q.Dist);
    z=z';
    xp=z(1,:);
    yp=z(2,:);
    A=ceil(sqrt(length(xp)));
    B=A;
    M=[];
    Base=1;
    return
...
if strcmp(Q.TightRep.Perm,'yes')~=1
    xp = round(1+(A*(z(1,:)-min(z(1,:)))/(max(z(1,:))-min(z(1,:)))));
    yp = round(1+(-B)*(z(2,:)-max(z(2,:)))/(max(z(2,:))-min(z(2,:))));
    A=max(xp);
    B=max(yp);
else
    [z,Cost,Time] = deepinsightTRep(z','Reference',Q.TightRep.Reference,...
        'EarlyStop',Q.TightRep.EarlyStop,'MaxIter',Q.TightRep.MaxIter,...
        'ShowRealTimeCostFunction',Q.TightRep.ShowRealTimeCostFunction,...
        'Dist',Q.Dist);
    z=z';
